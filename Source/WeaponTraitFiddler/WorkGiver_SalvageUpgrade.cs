...
namespace WeaponTraitFiddler
{
    public class WorkGiver_SalvageUpgrade : WorkGiver_Scanner
    {
        ...
        public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
        {
            ...
            Thing tableMachining = GenClosest.ClosestThingReachable(pawn.Position, pawn.Map, 
                ThingRequest.ForDef(WeaponTraitFiddlerUtils.GetWorkplaceThingDef()), 
                PathEndMode.InteractionCell, 
                TraverseParms.For(pawn, Danger.Some), 
                validator: (Predicate<Thing>) (thing => !thing.IsForbidden(pawn) && pawn.CanReserve((LocalTargetInfo) thing)));
